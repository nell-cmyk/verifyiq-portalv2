...
**Status:** SHIPPED 2026-05-13 **Phases:** 5-9 **Total Plans:** 8

## Overview

v1.1 turned the VerifyIQ Playwright suite into one operator-facing automation
runner while expanding authenticated portal coverage across Applications,
Activity, Audit Logs, Users, and Roles. The runner remains a thin wrapper over
committed Playwright tests, preserves native artifacts and exit behavior, and
keeps mutating workflows constrained to same-run automation-owned records.
...
- Keep `npm run test:portal` as a thin Playwright CLI wrapper instead of a
  custom browser automation runtime.
- Select runner targets with exact `@portal:*` Playwright grep tags.
- Require automation-owned same-run record registration before update, delete,
  cleanup, or residue diagnostics.
- Preserve MUT-05 Audit Logs same-run portal activity evidence and MUT-07 role
  edit as product-surface blockers instead of weakening assertions.
- Treat README as the canonical operator runbook for runner targets, auth
  recovery, artifact paths, and cleanup rules.

**Issues Resolved:**

- Fixed the Add Application required-applicant strict-locator failure by
  asserting against the inline `validation-error` surface.
- Added runner target coverage for all required public and authenticated portal
  areas.
- Added same-run mutation safety helpers and deep Users, Roles, Activity, and
  Audit Logs workflow coverage.
- Documented storage-state auth recovery, native Playwright artifacts, failure
  debug order, and same-run cleanup boundaries.

**Issues Deferred:**

- Audit Logs same-run portal user/role activity evidence remains blocked until
  the product exposes that activity in the Audit Logs surface.
- Role edit and reversible permission-toggle coverage remain blocked until the
  product exposes a visible role edit action.

**Technical Debt Incurred:**

- Phase 5, 6, 7, and 9 `*-VALIDATION.md` files contain stale validation metadata
  accepted as non-blocking milestone debt.

**Phase Archive:**

- Full phase execution history is archived in `v1.1-phases/`.
