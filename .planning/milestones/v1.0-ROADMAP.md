...
**Status:** SHIPPED 2026-05-11 **Phases:** 1-4 **Total Plans:** 7

## Overview

v1.0 built the VerifyIQ Portal automation MVP: deterministic Playwright coverage
for public loading, reCAPTCHA-safe authenticated setup, Add Application workflow
coverage, CI wiring, and regression triage operations.
...
- [x] **Phase 4: Regression Operations** - Improve reports, retries, triage, and
...
- [x] 04-01: Add secret-safe Playwright triage output.

**Wave 2 _(blocked on Wave 1 completion)_**

- [x] 04-02: Document regression maintenance workflow.
...
| 4. Regression Operations           | 2/2            | Complete | 2026-05-11 |
...
- TypeScript + Playwright Test is the automation source of truth.
- Storage-state auth is required because sandbox credential login is gated by
  reCAPTCHA.
- `agent-browser` stays optional exploration tooling, not a dependency or CI
  runtime.
- Claude Opus 4.7 implements planned changes first through
  `npm run ai:implement`; Codex reviews, verifies, and handles fallback.
- Secret-safe triage summaries supplement, but do not replace, native Playwright
  reports.

**Issues Resolved:**

- Replaced credential-only auth assumptions with env/local storage-state
  precedence and fresh-context validation.
- Corrected live Add Application UI assumptions for page identity and upload
  modal confirmation.
- Tightened created-record proof to avoid generic application-list false
  positives.
- Added CI behavior that stays green without auth secrets while still running
  full regression when storage state is available.

**Issues Deferred:**

- Browserbase/Stagehand remains deferred until local/CI Playwright is
  insufficient.
- Authenticated/full regression requires fresh local auth state or CI
  storage-state secret refresh.

**Technical Debt Incurred:**

- Phase 1 and Phase 2 summary files predate the current `requirements-completed`
  frontmatter convention.
- `authInputGuidance` and `defaultBaseUrl` exports are currently unused.

**Phase Archive:**

- Full phase execution history is archived in `v1.0-phases/`.
